...
echo('<a href="#" class="cerrar">CLOSE</a>');
...
echo('<a href="#" id="hsiguiente">Next</a>');

echo('<div id="foto">');
echo('<img src="" alt="Bitarqs Arquitectura" id="imagen" />');
echo('</div>');

echo('<a href="#" id="anterior">Back</a>');
echo('<a href="#" id="siguiente">Next</a>');
...
echo('<p><span>'. utf8_encode($var_title['labelen']).'</span>, '. utf8_encode($var_title['year']));

if ($var_title['ubicacion'] != NULL) {
	echo(', '. utf8_encode($var_title['location']));
}

if ($var_title['medidas'] != NULL) {
	echo(', '. utf8_encode($var_title['mesures']).' sq./ft.');
}
echo('</p></div>');
